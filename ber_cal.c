...
float ber_cal(const char* file1_path, const char* file2_path) {
    ...
    while ((byte1 = fgetc(file1)) != EOF && (byte2 = fgetc(file2)) != EOF) {
        // Count the number of bits in each character
    ...
    return (float)xor_sum/bitCount;;
}

int main() {
    //createRandomFile();
    //split_text_file("teratermabcd.log", "seperateforme123", "output1.txt");
    float result = ber_cal("output123.txt", "output1.txt");
    printf("BER results: %.2f\n", result);
    return 0;
}